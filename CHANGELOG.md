...
- `inspect --entity CANONICAL-ID` now renders one reviewed canonical record's
  ...
  without turning a record into a ranking or inferred profile.
...
- The public `catalog` now lists reviewed Research Problem IDs accepted by problem-aware discovery commands, so filter choices remain discoverable without reading canonical frontmatter.

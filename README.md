...
- Run `python3 scripts/research_landscape.py discover-problems --software SW-LAMMPS` to inspect the reviewed atomistic molecular-dynamics challenge through LAMMPS’s direct support assertion, without comparing simulation methods or research problems.
- Run `python3 scripts/research_landscape.py discover-problems --software SW-AFLOW` to inspect the reviewed high-throughput materials-screening challenge through AFLOW’s direct support assertion, without comparing screening strategies or research problems.
- Run `python3 scripts/research_landscape.py discover-problems --software SW-ASE` to inspect the reviewed atomistic structure-optimization challenge through ASE's direct support assertion, without comparing optimizers, calculators, or research problems.
- Run `python3 scripts/research_landscape.py discover-problems --open-source yes` to inspect problems with at least one directly supporting reviewed software record whose openness state is documented as `yes`; this is not a maintenance, quality, or fit signal.
...
- Run `python3 scripts/research_landscape.py inspect --entity SW-MACE` to inspect one reviewed canonical record's one-hop typed graph neighborhood and record-local evidence register without a ranking or profile inference.
- Run `python3 scripts/research_landscape.py discover-mentorship --mentorship-category onboarding-training` to inspect bounded public onboarding-process evidence and its limitations; this is not a “strong mentorship” ranking.
...
- Use `python3 scripts/research_landscape.py discover-pis --open-source yes` to find PIs with a documented development link to software whose openness state is documented as `yes`; this does not claim personal values, maintenance, or availability.
...
- Use `python3 scripts/research_landscape.py discover-ecosystems --language PROGRAMMING-LANGUAGE-CPP` to inspect ecosystems through explicit included-software implementation paths, without inferring team-wide language practice or ecosystem dominance.
